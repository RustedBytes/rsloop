...
## Granian HTTP benchmark

[`compare_granian.py`](compare_granian.py) measures the same Granian ASGI
application with stdlib asyncio, uvloop (winloop on Windows), and rsloop. It
uses [Granian's Python loop-registration API](https://github.com/emmett-framework/granian#asyncio-event-loop-initialization)
and verifies `/loop` before each run, so a mislabeled or unavailable loop cannot
silently produce measurements.

The load is generated by [`oha`](https://github.com/hatoo/oha), matching
Granian's own benchmark tooling. Each selected loop gets one warmup followed by
three 10-second runs by default. The table reports median requests per second
and median p50/p95/p99 latency across those runs.

Install `oha` first if it is not already available (`cargo install oha`,
`brew install oha`, or `winget install hatoo.oha`).

Build rsloop in release mode and run from the repository root:

```bash
uv run --with maturin maturin develop --release
uv run --with granian --with uvloop python benches/compare_granian.py
```

On Windows, replace `--with uvloop` with `--with winloop`. For a quick smoke
run and machine-readable output:

```bash
uv run --with granian --with uvloop python benches/compare_granian.py \
  --warmup-duration 1 --duration 2 --repeat 1 \
  --json-output benches/results/granian.json
```
